...
int main() {
    ...
    for(int i=1; i<=m; i++) {                           // Loop through each row
        for(int j=1; j<=i; j++) {                       // Loop through each column
            cout << "*" << " ";
        }
...
// print right triangle using *

/*
This C++ program prints a right-angled triangle pattern of asterisks (*) based on the number of rows m specified by the user. 
...
controlling the number of asterisks in each row.
